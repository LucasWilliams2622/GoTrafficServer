...
const { models: UserModel } = require("./UserModel");
...
const changePassword = async (phone, oldPassword, newPassword) => {
  try {
    return await UserService.changePassword(phone, oldPassword, newPassword);
  } catch (error) {
    throw error;
  }
};
...
const sendVerifyCode = async (email, subject, verifyCode) => {
  try {
    ...
    await db.users.update(
      { verificationCodeMail: verifyCode },
      { where: { email: email } }
    );
...
const verifyCode = async (email, verifyCode) => {
  try {
    const user = await db.users.findOne({ where: { email: email } });
    if (user) {
      if (user.verificationCodeMail === verifyCode) {
        await db.users.update(
...
const changeForgotPassword = async (email) => {
  try {
    return await UserService.changeForgotPassword(email);
  } catch (error) {
    return false;
  }
};
...
const sendMailForNewAccount = async (to) => {
  try {
    const mailOpitons = {
      ...
      to,
      subject: "Chào mừng bạn đến với Go Traffic",
      html: `<!DOCTYPE html>
                                                                ...
                                                                <table class="text_block block-2" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="mso-table-lspace: 0pt; mso-table-rspace: 0pt; word-break: break-word;">
                                                                    <tr>
                                                                        <td class="pad" style="padding-left:20px;">
                                                                            <div style="font-family: 'Trebuchet MS', Tahoma, sans-serif">
                                                                                <div class style="font-size: 14px; font-family: 'Montserrat', 'Trebuchet MS', 'Lucida Grande', 'Lucida Sans Unicode', 'Lucida Sans', Tahoma, sans-serif; mso-line-height-alt: 16.8px; color: #63c9cb; line-height: 1.2;">
                                                                                    <p style="margin: 0; font-size: 14px; mso-line-height-alt: 16.8px;"><span style="font-size:38px;">To Go Traffic App!</span></p>
                                                                                </div>
                                                                            </div>
                                                                        </td>
                                                                    </tr>
